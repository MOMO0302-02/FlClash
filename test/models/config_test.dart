...
void main() {
  ...
  group('VpnProps JSON round-trip', () {
    test('default values', () {
      ...
      // 2026-09-03 由 false 改成 true，和内核总开关 PatchClashConfig.ipv6 一起翻。
      // 只翻一个会让应用一直试 IPv6 一直 ENETUNREACH（VpnService.kt:172-183 把
      // IPv6 地址、::/0 路由、IPv6 DNS 三样锁在同一个 if 里），纯粹变慢。
  ...
  group('ProxiesStyleProps JSON round-trip', () {
    test('default values', () {
      ...
      // 默认排版已改为分组列表（对齐 Clash Party 桌面端），不再是标签页。
  ...
  group('ThemeProps JSON round-trip', () {
    test('default values', () {
      ...
      expect(props.appStyle, AppStyle.clashParty);
  ...
  group('Config composite serialization', () {
    ...
    test('full config round-trip', () {
      const config = Config(
        ...
        themeProps: ThemeProps(themeMode: ThemeMode.system),
