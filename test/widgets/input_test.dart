...
void main() {
  ...
  // 选项对话框现在是「一列行 + 选中的那条右边打对勾」，没有 Material 的圆点了，
  // 因此「重复点选中项会吐 null」那条 Material 怪癖也不复存在。要钉住的是它本来
  // 想守的东西：关掉对话框不能被当成选了什么——语言那种「跟随系统」是个合法的
  // null 选项，误报会把用户的设置改掉。
  testWidgets('ListItem.options reports nothing when the dialog is dismissed', (
    tester,
  ) async {
    var changed = false;
    ...
    // 对勾每一行都在，只是没选中的透明度为 0（始终占位，选中时才不会把标题
    // 挤窄），所以数「看得见的有几个」。
  ...
  testWidgets('ListInputPage adds, edits, selects, and deletes items', (
    ...
    // The Checkbox sits inside a ListTile whose internal AbsorbPointer blocks
    // direct taps. Invoke the onChanged callback programmatically instead.
  ...
  testWidgets('MapInputPage adds, reorders, selects, and deletes entries', (
    ...
    // Same AbsorbPointer issue — invoke onChanged programmatically.
    tester.widget<Checkbox>(find.byType(Checkbox).first).onChanged!(true);
    await tester.pumpAndSettle();
    await tester.tap(find.text('Select all'));
    await tester.pumpAndSettle();
