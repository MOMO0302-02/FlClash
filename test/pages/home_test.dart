...
import 'package:clash_party/manager/theme_manager.dart';
import 'package:clash_party/manager/window_manager.dart';
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      for (var width = 1180.0; width >= 500; width -= 20) {
        ...
        expect(find.byType(NavigationBar), findsNothing, reason: '$width');
      }

      // 关键是「窗口变窄不能把页面状态弄丢」：计数还在。
      expect(find.text('count: 1'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // 设置页分组标题和行可能同名（「应用程序」既是标题也是一行），
      // 限定在 ListTile 里找，标题不是 ListTile。
      final applicationItem = find.widgetWithText(ListTile, 'Application');
      ...
      final logItem = find.widgetWithText(ListTile, 'Logcat');
      await tester.scrollUntilVisible(
        logItem,
        500,
        scrollable: find.byType(Scrollable).first,
  ...
  testWidgets('mobile has no persistent navigation chrome', (tester) async {
    ...
    // 手机端的形态是「主页一堵磁贴，点磁贴进二级页面」，所以底部标签栏和
    // 侧边栏都不该存在。这条是在守着那个决定——哪天有人手滑把导航栏加回来，
    // 界面会突然多出一条，但除了这条测试没人会发现。
    expect(find.byType(NavigationBar), findsNothing);
    expect(find.byType(NavigationRail), findsNothing);
  ...
  testWidgets('switching home pages exits a generic search layer', (
    ...
    // 手机端没有常驻导航栏可点了，切页面直接推 provider——这条测试要钉的是
    // 「换页会退出搜索层」，跟用什么方式换页无关。
    container.read(currentPageLabelProvider.notifier).toPage(PageLabel.tools);
    await tester.pumpAndSettle();
