...
String _resolveCc(Target target) {
  ...
  final cc = p.join(entries.first.path, 'bin', target.ndkCcName);
  // Windows 的 NDK 里，无扩展名的那个编译器是 Unix shell 脚本，直接执行会报
  // “%1 不是有效的 Win32 应用程序”。同目录下有 .cmd 包装，必须改用它。
  if (Platform.isWindows) {
    final withCmd = File('$cc.cmd');
    if (withCmd.existsSync()) return withCmd.path;
  }
  return cc;
}
