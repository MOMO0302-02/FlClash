...
class CommonPopupMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // iOS 的上下文菜单本来就是玻璃，这是最能认出「这是 iOS」的一处。
    //
    // **只有 iOS 风格上玻璃**：Fluent 和默认风格的菜单是不透明的。玻璃是苹果
    // 那套设计语言，套到别的风格上是四不像。
    //
    // 阴影必须画在玻璃**外面**——`BackdropFilter` 会裁掉自身范围之外的绘制，
    // 阴影塞进去就没了（`_CapsuleSurface` 已经踩过这个坑）。
    if (tokens.style == AppStyle.cupertino) {
      return DecoratedBox(
    ...
    // 不用 Material 的 elevation：它的阴影带 surfaceTint 染色，会把纯黑底上的
    // 菜单染出一层蓝，跟卡片不是一个东西。改成和卡片同一套 rim + cardShadow。
    return DecoratedBox(
      decoration: BoxDecoration(
        borderRadius: radius,
        boxShadow: tokens.cardShadow,
