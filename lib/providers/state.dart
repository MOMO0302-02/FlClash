...
ColorScheme genColorScheme(
  ...
  // 配色推导方式和底色层都由界面风格决定，不再有单独的「主色 / 配色方案」设置。
  final tokens = ref.watch(
    themeSettingProvider.select((state) => AppStyleTokens.of(state.appStyle)),
  );
  if (color == null && ignoreConfig != true) {
    // if (globalState.corePalette != null) {
    //   return globalState.corePalette!.toColorScheme(brightness: brightness);
    // }
    // 一律用风格自己的种子色。曾经有一档 Material You 跟随系统壁纸取色，是唯一
    // 的例外；那一档已删。
    final seed = tokens.seed;
    return _applyStyle(
...
/// 把风格自带的底色层盖到 Material 推出来的配色上。
///
/// Material 从种子推出来的深色底是带色味的（蓝种子会推出藏青），而四套风格各有
/// 自己规定的底色：Clash Party 是纯黑 + zinc、Fluent 是云母灰、iOS 是纯黑 +
/// systemGray6。Material You 是唯一不覆盖的——它要的就是跟随系统取色。
///
/// 只覆盖「底色层」，主色和文字仍由种子推导。
ColorScheme _applyStyle(
  ColorScheme base,
  AppStyleTokens tokens,
  Brightness brightness,
) {
  // 主色直接钉死成风格的种子色，不用 Material 推出来的那个。
  //
  // 这是全 App 影响面最大的一处：Material 在深色下会把纯蓝种子推成很浅的淡紫蓝
  // （色调 80），而 `colorScheme.primary` 被 FilledButton、Switch、Checkbox、
  // Radio、Slider、进度条**全部**拿去用——于是满屏的主控件都不是设计稿那个
  // #006FEE。之前是各处自己去取 `styleTokens.accent` 绕开，绕不到的地方就偏色。
  // 从源头改掉，所有 Material 组件一起对上。
  //
  // 曾经 Material You 是例外（主色跟随系统壁纸），那一档已删，现在没有例外。
  final scheme = base.copyWith(
    primary: tokens.seed,
    onPrimary: const Color(0xFFFFFFFF),
    // 危险色同理：Material 从蓝种子推出来的 error 是偏橙的红，桌面端是偏
    // 玫红的 #F31260。删除按钮、断开连接、校验失败全在用它。
    error: AppStyleTokens.danger,
    onError: AppStyleTokens.onDanger,
  );
  // 深浅两套底色层各自独立：桌面端浅色下卡片和页面都是纯白（HeroUI light 主题里
  // `background` 和 `content1` 都是 #FFFFFF，见
  // `@heroui/theme/dist/chunk-FFKHVGFX.mjs:11-44`），深色下才是黑底 + zinc 卡片。
  // 之前只覆盖深色，浅色一直用 Material 从种子推出来的带色味的灰。
  final surfaces = tokens.surfacesFor(brightness);
  if (surfaces == null) {
    return scheme;
  }
  return surfaces.applyTo(scheme);
}
...
SharedState sharedState(Ref ref) {
  ...
  final appSettingVM3 = ref.watch(
    appSettingProvider.select(
      (state) => VM3(
        state.onlyStatisticsProxy,
        state.testUrl,
        state.showNotificationSpeed,
      ),
    ),
  );
  ...
  final showNotificationSpeed = appSettingVM3.c;
...
/// Smart 的全部下发参数，含运行时的网络类型。
///
/// 为什么不直接用 `appSettingProvider.smartOptions`：「仅 Wi-Fi 更新」那道闸要看
/// **当前是不是 Wi-Fi**，那是运行时状态，不在设置里。把它并进来还有一个好处——
/// 网络类型一变，这个 provider 就变，`CoreManager` 那条监听会自动重新下发配置，
/// 于是切到流量时内核的自动更新真的会停掉，而不是等到下次改设置才生效。
@riverpod
SmartOptions smartOptionsState(Ref ref) {
  final smartOptions = ref.watch(
    appSettingProvider.select((state) => state.smartOptions),
  );
  final onWifi = ref.watch(onWifiProvider);
  return smartOptions.copyWith(onWifi: onWifi);
}
