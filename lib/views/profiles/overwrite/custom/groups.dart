...
import 'package:clash_party/models/models.dart' hide FileInfo;
  ...
  Widget build(BuildContext context) {
    ...
    return CommonScaffold(
      ...
      body: proxyGroups.isEmpty
          // 空状态直接给动作：顶栏那个「添加」按钮小又靠边，第一次进来的人
          // 容易找不到。这里没有浮动按钮，不会重复。
          ? NullStatus(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Icon(
        Icons.check,
        size: 20.ap,
        // 「已选中/已包含」在本项目里一律是主色的对勾——和代理页设置面板、
        // 覆写模式那几组一致。原来是一枚 harmonize 过的绿色圆圈勾，
        // 既不是设计语言里的颜色，形状也和别处的勾不一样。
...
/// 计数角标。桌面端是 HeroUI `variant="bordered"` 的 Chip：空心主色边 + 同色
/// 文字、内部透明。原来是一张 Material `Card`（灰底实心块，圆角还写死 14）。
