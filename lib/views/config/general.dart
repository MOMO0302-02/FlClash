  ...
  Widget build(BuildContext context) {
    ...
    return CommonDialog(
      ...
      child: Form(
        ...
        // 自定义 UA 留空时下面会长出一行红字，对话框跟着变高。原来是一下子弹开，
        // 手指还停在输入框上、整个框先跳一截，很像出了错。让它长过去，红字就是
        // 「多出来的一行」而不是「屏幕抖了一下」。
        child: AnimatedSize(
...
// Smart 选路的开关搬去了 `smart.dart`。它已经不是孤零零一个开关，而是一整页
// （模型、数据收集、策略、容差…），继续挂在"基础配置"里既不好找也放不下。
...
final generalItems = <Widget>[
  ...
  const Ipv6Item(),
  const AllowLanItem(),
  // 紧跟「允许局域网」，和桌面版 mihomo.tsx 的相邻关系一致：开了门就得能看到锁
  // 在哪儿。没设密码时这一条自己会变成警告。
  const LanAuthItem(),
    ...
    return CommonDialog(
      ...
      child: Form(
        ...
        child: Padding(
          ...
          // 展开「更多端口」时对话框自己长高，四个字段是被拉出来的。
          // 时长和曲线改从 Motion 取：原来写的是本文件独有的一组值，和别处的
          // 展开收起对不上。
          child: AnimatedSize(
