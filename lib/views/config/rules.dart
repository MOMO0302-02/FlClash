...
import 'package:clash_party/features/features.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return CommonPopScope(
      ...
      child: BaseScaffold(
        ...
        body: rules.isEmpty
            // 空状态是邀请用户做下一件事的地方。原来只写一句「暂无规则」，
            // 而唯一的「添加」按钮在顶栏右上角，不一定被看到。
            ? NullStatus(
                label: appLocalizations.nullTip(appLocalizations.rule),
                description: appLocalizations.standardModeDesc,
                illustration: const RuleEmptyIllustration(),
                action: FilledButton.tonal(
                  onPressed: () {
                    _handleAddOrUpdate();
                  },
                  child: Text(appLocalizations.addRule),
                ),
              )
            : ReorderableList(
