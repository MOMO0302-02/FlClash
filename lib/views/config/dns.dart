...
class OverrideItem extends ConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    // 这条原来是裸的一行，通栏平铺在两张分组卡片上面，一眼看出是两套样式。
    // 套进无标题的分组卡片后，整页三块都是同一种圆角卡片。
    //
    // 开关一切，下面那行说明会整句换掉（「会覆盖订阅的 DNS」↔「订阅自己写了
    // DNS 时不生效」），两句长短不同、折行数也不同，卡片高度跟着变。原来是瞬间
    // 变高变矮、底下两组一起跳；AnimatedSize 让它长过去，说明文字就是「改了一
    // 句话」而不是「页面动了一下」。
    return AnimatedSize(
        ...
        children: generateSection(
          items: [
            ListItem.toggle(
              ...
              // 关掉时把话说清楚：下面整页设置只有在「订阅本身没写 dns」时才生效
              // （`task.dart` 的条件是 `overrideDns || !isEnableDns`）。原来关着
              // 的时候下面照样能改，改完不生效，界面上一个字的提示都没有。
              subtitle: Text(
                override
                    ? appLocalizations.overrideDnsDesc
                    : appLocalizations.overrideDnsOffTip,
              ),
              value: override,
