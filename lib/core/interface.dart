...
abstract class CoreHandlerInterface with CoreInterface {
  Future<T?> _invokeMethod<T>({
    ...
    return await utils.handleWatch(
      onStart: () {
        // 不能直接打 $arguments：sideLoadExternalProvider 的 'data' 是整份
        // proxy-provider 文件，节点密码和 UUID 都在里面。
        commonPrint.log(
          'Invoke method ${method.name} ${DateTime.now()} '
          '${describeCoreArguments(arguments)}',
        );
      },
      function: () async {
