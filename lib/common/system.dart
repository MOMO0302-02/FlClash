...
import 'package:clash_party/core/desktop/helper_client.dart';
...
class System {
  ...
  Future<int> init() async {
    // 顺带算一次「这台机器扛得住多重的实时绘制」。判据取自设备自己的回答
    // （低内存标记、物理内存、64 位 ABI），不看安卓版本号——版本号和性能没关系。
    await RenderCapability.init();
  ...
  /// 连续多少次启动没走完。0 表示上次好好的。
  Future<int> consumeStartupFailureStreak() async {
    if (!isAndroid) return 0;
    return await app?.consumeStartupFailureStreak() ?? 0;
  }
