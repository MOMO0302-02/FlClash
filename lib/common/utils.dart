...
class Utils {
  ...
  /// 延迟分档着色。
  ///
  /// 原先是「600ms 以下一律绿」——40ms 和 500ms 长得一样好，等于没有区分度，
  /// 而这两者的实际体验差着一个数量级。改成三档，让扫一眼就能挑出真正快的：
  ///
  /// - < 150ms  好：同区域或邻近区域的节点
  /// - 150–400  可用：跨区但还能接受
  /// - > 400ms  差：能连但会明显卡
  ...
  Color? getDelayColor(int? delay) {
    ...
    if (delay < 0) return const Color(0xFFD96B6B);
    if (delay < 150) return const Color(0xFF7ED4A6);
    if (delay < 400) return const Color(0xFFE8B23A);
