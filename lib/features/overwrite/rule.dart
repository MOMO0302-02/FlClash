...
class RuleItem extends StatelessWidget {
  ...
  VM2<bool, Color?> _checkInvalid(BuildContext context) {
    if (rule.ruleAction != RuleAction.SUB_RULE) {
      ...
      // 走统一的语义色。原来是 Material 的 green / orange 再
      // `harmonizeWith` 往主色方向拧——四种风格四种主色，就拧出四套颜色，
      // 而同样表达"放行/拦截"的地方在别处用的又是另一组值。
      if (ruleTarget.toUpperCase() == 'DIRECT') {
        return const VM2(false, AppStyleTokens.success);
      } else if (ruleTarget.toUpperCase() == 'REJECT') {
        return const VM2(false, AppStyleTokens.warning);
...
class _AddOrEditRuleDialogState extends State<AddOrEditRuleDialog> {
  ...
  @override
  void dispose() {
    // 这两个控制器是本 State 自己 new 出来的，必须自己释放。原来整个类连
    // dispose 都没有，于是每开一次「添加 / 编辑规则」弹窗就漏两个
    // TextEditingController（各自还挂着输入框注册的监听器）——规则页是可以
    // 反复开关的，漏的量随使用次数一直涨。
